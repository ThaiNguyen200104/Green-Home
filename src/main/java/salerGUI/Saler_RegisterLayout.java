...
public class Saler_RegisterLayout extends JPanel {
	...
	public Saler_RegisterLayout() {
			...
			gl_centerPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(Alignment.TRAILING, gl_centerPanel.createSequentialGroup()
					...
					.addGroup(gl_centerPanel.createParallelGroup(Alignment.TRAILING)
						.addComponent(applicationPanel, Alignment.LEADING, GroupLayout.DEFAULT_SIZE, 1011, Short.MAX_VALUE)
						.addGroup(gl_centerPanel.createSequentialGroup()
							.addComponent(btnRoomAllocation)
		...
		gl_showRegisterPanel.setHorizontalGroup(
			gl_showRegisterPanel.createParallelGroup(Alignment.TRAILING)
		...
		gl_showRegisterPanel.setVerticalGroup(
			gl_showRegisterPanel.createParallelGroup(Alignment.LEADING)
		...
		centerShowPanel.add(lblSubTotal, gbc_lblSubTotal);
				...
				centerShowPanel.add(lblOthers, gbc_lblOthers);
						...
						lblMaintainSecurityNotify = new JLabel(
								"Security fee is 100.000. If customer didn't call for maintenance, maintain fee = 0");
...
//		---------------------------------------------------------------------------------
	...
	public void loadRegister() {
		var LR_model = new DefaultTableModel() {
			...
			public Class getColumnClass(int column) {
				switch (column) {
				case 0: return String.class; 	// cus_fullname
				case 1: return LocalDate.class; // cus_dob
				case 2: return String.class; 	// contact_no
				case 3: return String.class; 	// cus_address
				case 4: return String.class; 	// mng_fullname
				case 5: return Integer.class; 	// deposit
				case 6: return LocalDate.class; // register_day
				case 7: return Integer.class; 	// register_no
		...
		new RegisterDAO().show_register()
			.forEach(sr -> LR_model.addRow(new Object[] { sr.getSr_cus_fullname(), sr.getSr_cus_dob(),
				sr.getSr_cus_address(), sr.getSr_cus_contactNo(), sr.getSr_mng_fullname(), 
				sr.getSr_deposit(), sr.getSr_register_day(), sr.getSr_register_no() }));
	...
	protected void showRegisterTableMouseClicked(MouseEvent e) {
		...
		try {
			...
			try {
				showDob.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(showRegisterTable.getValueAt(rowIndex, 1).toString()));
			...
			try {
				showRegDate.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(showRegisterTable.getValueAt(rowIndex, 6).toString()));
			} catch (ParseException e1) {
				e1.printStackTrace();
			}
	...
	public void showRoomId() {
		...
		SRI_roomModel.addAll(SRI_regisDAO.getRoomId());
	...
	public void addRoomId() {
		...
		ARID_roomModel.addAll(ARID_regisDAO.getRoomId());
	...
	protected void cbbAddCusNameItemStateChanged(ItemEvent e) {
		...
		try {
			addDob.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(cbb_cusInfo.get(cbbAddCusName.getSelectedIndex()).getCus_dob().toString()));
		} catch (ParseException e1) {
			e1.printStackTrace();
		}
	}
