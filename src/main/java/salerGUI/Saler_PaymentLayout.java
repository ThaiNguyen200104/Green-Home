...
public class Saler_PaymentLayout extends JPanel {
	...
	public Saler_PaymentLayout() {
		...
		lblMaintainSecurityNotify = new JLabel("Security fee is 100.000. If customer didn't call for maintenance, maintain fee = 0");
		...
		lblMaintainSecurityNotify_1 = new JLabel("Security fee is 100.000. If customer didn't call for maintenance, maintain fee = 0");
		...
		lblMaintainSecurityNotify_2 = new JLabel("Security fee is 100.000. If customer didn't call for maintenance, maintain fee = 0");
...
//		---------------------------------------------------------------------------------
	...
	public void loadShowRoomId() {
		...
		LSRI_payModel.addAll(LSRI_payDAO.getShowRoomId());
	}
	...
	public void loadRoomIdForMonthlyCheck() {
		...
		LRIFMC_checkOutModel.addAll(LRIFMC_payDAO.getShowRoomId());
	...
	protected void cbbMCIdItemStateChanged(ItemEvent e) {
		...
		for (var item : MC_getRA) {
			if(MC_roomID.equals(item.getRoom_id())) {
				try {
					...
					txt_MC_GetRANO.setText(String.valueOf(item.getRa_no()));
		...
		for (var item2 : getRoomPrice) {
			if(MC_roomID.equals(item2.getRoom_id())) {
				txtMC_R_Price.setText(String.format("%02f", item2.getRoom_price()).toString());
			}
		}
	...
	protected void txtMC_R_PriceKeyReleased(KeyEvent e) {
		try {
			...
			var r_total = r_price;
			txtMC_R_Total.setText(String.format("%02f", r_total).toString());
	...
	public void loadRoomIdForCheckOut() {
		...
		LRIFCO_checkOutModel.addAll(LRIFCO_payDAO.getShowRoomId());
